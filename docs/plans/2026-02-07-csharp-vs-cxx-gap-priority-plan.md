...
- P1-2: `MoveOverhead` / `MinimumThinkingTime` / `SlowMover` / `RoundUpToFullSecond` / `NetworkDelay` / `NetworkDelay2` / `MaxMovesToDraw` を実装済み。`ponderhit` 基準の停止判定も対応済み。
- P1-3: `Incremental == Recompute` の検証を拡張（白側成り・白側打ち・玉取りfallback）。NNUE parity strict 実行フロー、ケース網羅チェック、失敗時診断ログを整備済み。
- P2-1: `Threads > 1` の LazySMP（root並列）を導入済み。`go`/`go infinite`/`go ponder` で適用し、winner選出・安全fallback・秒読み超過回帰を実装済み。
- P2-2: 主要USI互換オプションの段階導入を実装済み（`USI_Hash`/`USI_Ponder`/`Clear Hash`/`USI_OwnBook`/`BookFile`/`USI_LimitStrength`/`USI_Elo`/`USI_ShowCurrLine`/`USI_ShowRefutations`）。`info` へ `multipv 1` を追加済み。

### 進行中
- P1-3: parityケース品質の継続改善（対局ログ由来ケースの追加、分布最適化）。
- P2-1: split point 方式への移行可否評価（現行はroot並列）。

### 未完了
- P2-1: split point 並列、共有TT/履歴の同期最適化、スレッド数増加時のスケーリング改善。
- P2-2: C++同等のチューニング/運用オプション体系への最終収束（README/runbook/tuning反映を含む）。

### 次アクション
1. `Threads > 1` のsplit point最小設計を作成し、root並列との差分を評価する。
2. スレッド数別（1/2/4）でNPSと秒読み超過率の基準値を固定する。
