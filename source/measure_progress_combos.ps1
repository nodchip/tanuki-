...
foreach ($c in $combos) {
    ...
    Write-Host ""
    Write-Host ("[{0:HH:mm:ss}] [{1}/{2}] START: {3}" -f (Get-Date), $comboIndex, $totalCombos, $label)
    ...
    if (!(Test-Path ".\YaneuraOu-by-gcc.exe")) {
        throw "YaneuraOu-by-gcc.exe not found after build. combo=$label"
    }

    for ($r = 1; $r -le 5; $r++) {
        Write-Host ("[{0:HH:mm:ss}] [{1}/{2}] Run {3}/5 running..." -f (Get-Date), $comboIndex, $totalCombos, $r)
        ...
        if (-not $line) {
            throw "Nodes/second not found. log=$log"
        }
        if ($line.Line -notmatch "([0-9,]+)") {
            throw "Failed to parse Nodes/second. log=$log"
        }

        $nps = ($matches[1] -replace ",", "")
        "$($c.n)`t$($c.d)`t$($c.t)`t$r`t$nps" | Add-Content -Path $OutputTsv -Encoding utf8

        Write-Host ("[{0:HH:mm:ss}] [{1}/{2}] Run {3}/5 done: NPS={4}" -f (Get-Date), $comboIndex, $totalCombos, $r, $nps)
    }

    Write-Host ("[{0:HH:mm:ss}] [{1}/{2}] DONE: {3}" -f (Get-Date), $comboIndex, $totalCombos, $label)
}

Write-Host ""
Write-Host ("[{0:HH:mm:ss}] All combinations completed." -f (Get-Date))
