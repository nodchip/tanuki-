...
namespace {
#if defined(GOAL_SHOGITEST_COMPAT)
constexpr int kDefaultFVScale = 24;
#else
...
#if defined(GOAL_SHOGITEST_COMPAT)
// 公開 source に固定した同一の進行度係数を使う。private NNUE は埋め込まない。
INCBIN(GoalProgress, "progress.bin");
#endif
...
#if defined(SFNNwoPSQT) && NNUE_SFNN_PROGRESS_BUCKETS != 1
#if defined(GOAL_SHOGITEST_COMPAT)
        const std::string progress_bytes(reinterpret_cast<const char*>(gGoalProgressData),
                                         gGoalProgressSize);
        std::istringstream progress_stream(progress_bytes, std::ios::in | std::ios::binary);
#else
