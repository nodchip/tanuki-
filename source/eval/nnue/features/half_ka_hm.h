...
class HalfKA_hm {
   public:
    ...
    // 評価関数ファイルに埋め込むハッシュ値
    static constexpr std::uint32_t kHashValue = 0x5f134cb9u ^ (AssociatedKing == Side::kFriend);
    // 特徴量の次元数
    ...
    static constexpr TriggerEvent kRefreshTrigger = (AssociatedKing == Side::kFriend)
                                                    ? TriggerEvent::kFriendKingMoved
    ...
    static void AppendChangedIndices(const Position& pos,
                                     Color           perspective,
                                     IndexList*      removed,
                                     IndexList*      added);
   ...
   private:
    // 駒の情報を取得する
    static void
    GetPieces(const Position& pos, Color perspective, BonaPiece** pieces, Square* sq_target_k);
};
