...
inline IndexType HalfKA_hm<AssociatedKing>::MakeIndex(Square sq_k, BonaPiece p) {
    if (sq_k >= SQ_61)
    {
        ...
        if (p >= fe_hand_end)
        {
            ...
            sq_p                  = Mir(sq_p);
            p = static_cast<BonaPiece>(fe_hand_end + piece_index * static_cast<IndexType>(SQ_NB)
                                       + sq_p);
        }
    }
    // 後手玉は自玉と同じPLANEに持っていく
    return static_cast<IndexType>(e_king) * static_cast<IndexType>(sq_k)
         + static_cast<IndexType>(p >= e_king ? p - SQ_NB : p);
}

// 駒の情報を取得する
template<Side AssociatedKing>
inline void HalfKA_hm<AssociatedKing>::GetPieces(const Position& pos,
                                                 Color           perspective,
                                                 BonaPiece**     pieces,
                                                 Square*         sq_target_k) {
    *pieces =
      (perspective == BLACK) ? pos.eval_list()->piece_list_fb() : pos.eval_list()->piece_list_fw();
...
template<Side AssociatedKing>
void HalfKA_hm<AssociatedKing>::AppendActiveIndices(const Position& pos,
                                                    Color           perspective,
                                                    IndexList*      active) {
...
template<Side AssociatedKing>
void HalfKA_hm<AssociatedKing>::AppendChangedIndices(const Position& pos,
                                                     Color           perspective,
                                                     IndexList*      removed,
                                                     IndexList*      added) {
    BonaPiece* pieces;
